...
### 2. Seurat anchor transfer in formal runs

`seurat_anchor_transfer` is now the formal Seurat-based comparator. Small or
unstable runs may still use different internal backends such as
`TransferData-only`, but formal main tables should report the actual backend
explicitly rather than relabeling the method as native Azimuth.
