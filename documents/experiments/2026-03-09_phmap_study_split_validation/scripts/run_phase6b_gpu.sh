...
MANIFEST_SCRIPT="${SCRIPT_DIR}/generate_phase6b_manifests.py"
COLLECT_SCRIPT="${SCRIPT_DIR}/collect_phase6b_results.py"
TRAIN_INDEX="${REPO_ROOT}/documents/experiments/2026-03-09_phmap_study_split_validation/manifests/phase6b/train_manifest_index.json"
TMP_ROOT="/tmp/atlasmtl_benchmarks/2026-03-10/phmap_study_split_phase6b"
...
PY
  ...
  if has_run_outputs "${run_dir}"; then
    echo "=== skip phase6b train ${config_name} seed_${seed} ==="
    continue
  fi
  echo "=== phase6b train ${config_name} seed_${seed} ==="
  "${PYTHON_BIN}" "${RUNNER}" \
