...
OUT_ROOT = DOSSIER_ROOT / "manifests" / "phase3_tradeoff"
PREP_ROOT = Path(
...
def _configured_manifest(
    ...
    *,
    ...
    payload = _base_manifest(point, seed=seed)
    payload["train"] = {
    ...
    payload["optimization_stage"] = "phase3_tradeoff"
...
def main() -> None:
    ...
    train_rows = []
    predict_rows = []
    for config_name, train_overrides in CONFIGS.items():
        train_payload = _configured_manifest(
            "build_100000_eval10k",
            config_name=f"atlasmtl_study_split_phase3_{config_name}_train",
            rules=rules,
            train_overrides=train_overrides,
            enforce_hierarchy=True,
            seed=seed,
        )
        train_path = OUT_ROOT / "train" / config_name / "atlasmtl_phase3_train.yaml"
        ...
        for point in POINTS:
            for hierarchy_setting, enforce_hierarchy in (("on", True), ("off", False)):
                predict_payload = _configured_manifest(
                    point,
                    config_name=f"atlasmtl_study_split_phase3_{config_name}_{point}_hierarchy_{hierarchy_setting}",
                    rules=rules,
                    train_overrides=train_overrides,
                    enforce_hierarchy=enforce_hierarchy,
                    seed=seed,
                )
                predict_path = OUT_ROOT / "predict" / config_name / point / f"hierarchy_{hierarchy_setting}" / "atlasmtl_phase3_predict.yaml"
