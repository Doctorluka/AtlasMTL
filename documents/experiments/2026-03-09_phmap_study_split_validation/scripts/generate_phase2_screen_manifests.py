...
OUT_ROOT = DOSSIER_ROOT / "manifests" / "phase2_screen"
PREP_ROOT = Path(
...
SCREEN_CONFIGS: Dict[str, Dict[str, Any]] = {
    "lv4strong_baseline": {},
...
def _configured_manifest(
    ...
    payload["optimization_stage"] = "phase2_screen"
...
def main() -> None:
    ...
    train_rows = []
    predict_rows = []
    for config_name, overrides in SCREEN_CONFIGS.items():
        train_payload = _configured_manifest(
            "build_100000_eval10k",
            f"atlasmtl_study_split_phase2_screen_{config_name}_train",
            rules,
            overrides,
        )
        train_path = OUT_ROOT / "train" / config_name / "atlasmtl_phase2_train.yaml"
        ...
        for point in POINTS:
            predict_payload = _configured_manifest(
                point,
                f"atlasmtl_study_split_phase2_screen_{config_name}_{point}",
                rules,
                overrides,
            )
            predict_path = OUT_ROOT / "predict" / config_name / point / "atlasmtl_phase2_predict.yaml"
