...
MANIFEST_SCRIPT="${SCRIPT_DIR}/generate_phase3_tradeoff_manifests.py"
TRAIN_INDEX="${REPO_ROOT}/documents/experiments/2026-03-09_phmap_study_split_validation/manifests/phase3_tradeoff/train_manifest_index.json"
PREDICT_INDEX="${REPO_ROOT}/documents/experiments/2026-03-09_phmap_study_split_validation/manifests/phase3_tradeoff/predict_manifest_index.json"
TMP_ROOT="/tmp/atlasmtl_benchmarks/2026-03-10/phmap_study_split_phase3_tradeoff"
...
PY
  ...
  if has_run_outputs "${run_dir}"; then
    echo "=== skip phase3 train ${config_name} ==="
    continue
  fi
  echo "=== phase3 train ${config_name} ==="
  "${PYTHON_BIN}" "${RUNNER}" \
...
PY
  ...
  mkdir -p "${run_dir}"
  if has_run_outputs "${run_dir}"; then
    echo "=== skip phase3 predict ${config_name} ${point} hierarchy_${hierarchy_setting} ==="
    continue
  fi
  echo "=== phase3 predict ${config_name} ${point} hierarchy_${hierarchy_setting} ==="
  "${PYTHON_BIN}" "${RUNNER}" \
    --dataset-manifest "${manifest_path}" \
    --output-dir "${run_dir}" \
