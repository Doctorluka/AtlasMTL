...
ARTIFACT_DIR = DOSSIER_ROOT / "artifacts" / "phase7a_auto_reranker_pipeline"
RESULTS_DIR = DOSSIER_ROOT / "results_summary" / "phase7a_auto_reranker_pipeline"
...
def _summarize_variant(*, variant_name: str, point: str, result, truth_df: pd.DataFrame, hierarchy_rules):
...
def main() -> None:
    ...
    ranking_path = ARTIFACT_DIR / "hotspot_ranking.json"
    ...
    per_parent_summary_path = ARTIFACT_DIR / "per_parent_reranker_summary.csv"
    ...
    plan_path = ARTIFACT_DIR / "refinement_plan.json"
    plan.save(plan_path)
    ...
    for point in POINTS:
        ...
        for variant_name, result in (("baseline", base_result), ("auto_reranker_top6", auto_result)):
    ...
    comparison_df.to_csv(RESULTS_DIR / "phase7a_auto_reranker_pipeline.csv", index=False)
    pd.DataFrame(breakdown_rows).to_csv(RESULTS_DIR / "phase7a_auto_reranker_parent_child_breakdown.csv", index=False)

    target_base = comparison_df[
        (comparison_df["variant_name"] == "baseline") & (comparison_df["point"] == TARGET_POINT)
    ].iloc[0]
    target_auto = comparison_df[
        (comparison_df["variant_name"] == "auto_reranker_top6") & (comparison_df["point"] == TARGET_POINT)
    ].iloc[0]
    guardrail_decision = {
        "point": f"{TARGET_POINT} + hierarchy_on",
        "passed": bool(
            (target_auto["anno_lv4_macro_f1"] >= target_base["anno_lv4_macro_f1"])
            and (target_auto["full_path_accuracy"] >= target_base["full_path_accuracy"])
            and (
                target_auto["parent_correct_child_wrong_rate"]
                <= target_base["parent_correct_child_wrong_rate"]
            )
        ),
        "base_metrics": {
            "anno_lv4_macro_f1": float(target_base["anno_lv4_macro_f1"]),
        ...
        "refined_metrics": {
            "anno_lv4_macro_f1": float(target_auto["anno_lv4_macro_f1"]),
    ...
    (ARTIFACT_DIR / "guardrail_decision.json").write_text(
        json.dumps(guardrail_decision, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    ...
    summary_lines = [
        ...
        f"- selected parents: `{', '.join(selected_parents)}`",
        f"- guardrail passed: `{guardrail_decision['passed']}`",
        "",
        "## Outputs",
        "",
    ...
    (RESULTS_DIR / "phase7a_auto_reranker_pipeline.md").write_text(
        "\n".join(summary_lines),
        encoding="utf-8",
    )
