...
MANIFEST_SCRIPT="${SCRIPT_DIR}/generate_phase1_manifests.py"
TRAIN_INDEX="${REPO_ROOT}/documents/experiments/2026-03-09_phmap_study_split_validation/manifests/phase1/train_manifest_index.json"
PREDICT_INDEX="${REPO_ROOT}/documents/experiments/2026-03-09_phmap_study_split_validation/manifests/phase1/predict_manifest_index.json"
TMP_ROOT="/tmp/atlasmtl_benchmarks/2026-03-09/phmap_study_split_phase1"
...
PY
  ...
  if has_run_outputs "${run_dir}"; then
    echo "=== skip existing study-split phase1 train ${config_name} ==="
    continue
  fi
  echo "=== study-split phase1 train ${config_name} ==="
  "${PYTHON_BIN}" "${RUNNER}" \
...
rows = json.load(open(sys.argv[1]))
for item in rows:
    print(f"{item['config_name']}\t{item['point']}\t{item['hierarchy_setting']}\t{item['predict_manifest_path']}")
PY
  [[ -n "${manifest_path}" ]] || continue
  ...
  mkdir -p "${run_dir}"
  if has_run_outputs "${run_dir}"; then
    echo "=== skip existing study-split phase1 predict ${config_name} ${point} hierarchy_${hierarchy_setting} ==="
    continue
  fi
  echo "=== study-split phase1 predict ${config_name} ${point} hierarchy_${hierarchy_setting} ==="
  "${PYTHON_BIN}" "${RUNNER}" \
    --dataset-manifest "${manifest_path}" \
    --output-dir "${run_dir}" \
