...
PY
  [[ -n "${manifest}" ]] || continue
  out_dir="/tmp/atlasmtl_benchmarks/2026-03-09/atlasmtl_low_cost_optimization/${dataset}/benchmark/stage_a/gpu/${point}/${config}"
  mkdir -p "${out_dir}"
  if should_skip_point "${out_dir}"; then
    echo "=== skip existing stage_a gpu ${dataset} ${point} ${config} ==="
    continue
  fi
  echo "=== stage_a gpu ${dataset} ${point} ${config} ==="
  "${PYTHON_BIN}" "${RUNNER}" \
