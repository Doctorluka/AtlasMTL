...
for dataset in "${DATASETS[@]}"; do
  out_root="/tmp/atlasmtl_benchmarks/2026-03-06/formal_third_wave/${dataset}/benchmark/formal_main/cpu_core"
  mkdir -p "${out_root}"
  while IFS=$'\t' read -r manifest point; do
    [[ -n "${manifest}" ]] || continue
    out_dir="${out_root}/${point}"
    if should_skip_point "${out_dir}"; then
      echo "=== skip existing ${dataset} CPU core: ${point} ==="
      continue
    fi
    echo "=== ${dataset} CPU core: ${point} ==="
    "${PYTHON_BIN}" "${RUNNER}" \
      --dataset-manifest "${manifest}" \
      --output-dir "${out_dir}" \
      --device cpu \
      --methods "${CPU_METHODS[@]}"
  done < <(emit_points "${dataset}" "cpu")
done
