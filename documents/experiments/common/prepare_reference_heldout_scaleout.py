...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prep-manifest", required=True)
    parser.add_argument("--runtime-manifest-10k", required=True)
    parser.add_argument("--runtime-manifest-5k", required=True)
    ...
    parser.add_argument("--predict-size-10k", type=int, default=10000)
    parser.add_argument("--predict-size-5k", type=int, default=5000)
    parser.add_argument("--seed", type=int, default=2026)
    parser.add_argument("--n-candidates", type=int, default=128)
    parser.add_argument("--warning-build-label-min", type=int, default=25)
    parser.add_argument("--warning-predict-label-min", type=int, default=10)
...
def _subset_from_predict_pool(adata, subset_size: int, seed: int):
    if adata.n_obs < subset_size:
        raise ValueError(f"predict pool subset size {subset_size} exceeds available cells {adata.n_obs}")
    if adata.n_obs == subset_size:
        return adata.copy()
    import numpy as np

    rng = np.random.default_rng(seed)
    chosen = np.sort(rng.choice(adata.n_obs, size=subset_size, replace=False))
...
def main() -> None:
    args = parse_args()
    prep_manifest = _load_manifest(args.prep_manifest)
    manifest_10k = _load_manifest(args.runtime_manifest_10k)
    manifest_5k = _load_manifest(args.runtime_manifest_5k)
    preprocess_config = _build_preprocess_config(prep_manifest)
    source_h5ad = Path(args.source_h5ad).expanduser().resolve()
    ...
    split_plan = make_group_split_plan(
        ...
        predict_size=args.predict_size_10k,
        seed=args.seed,
        n_candidates=args.n_candidates,
        warning_build_label_min=args.warning_build_label_min,
        warning_predict_label_min=args.warning_predict_label_min,
    )
    split_plan["domain_key"] = args.domain_key
    split_plan["dataset_name"] = prep_manifest.get("dataset_name")
    split_plan["source_h5ad"] = str(source_h5ad)
    split_plan["predict_size_10k"] = int(args.predict_size_10k)
    split_plan["predict_size_5k"] = int(args.predict_size_5k)
    ...
    predict_10k_canonical = materialized["predict_adata"]
    predict_5k_canonical = _subset_from_predict_pool(
        predict_10k_canonical,
        subset_size=args.predict_size_5k,
        seed=args.seed + 2,
    )
    split_summary = dict(materialized["split_summary"])
    split_summary["domain_key"] = args.domain_key
    split_summary["dataset_name"] = prep_manifest.get("dataset_name")
    split_summary["source_h5ad"] = str(source_h5ad)
    split_summary["predict_5k_subset_cells"] = int(predict_5k_canonical.n_obs)
    ...
    predict_10k_prepared, predict_10k_align_report = align_query_to_feature_panel(
        predict_10k_canonical,
        feature_panel,
        preprocess_config,
    )
    predict_5k_prepared, predict_5k_align_report = align_query_to_feature_panel(
        predict_5k_canonical,
        feature_panel,
        preprocess_config,
    )
    ...
    predict_10k_report = _compose_preprocess_report(
        config=preprocess_config,
        canonical_report=full_canonical_report,
        counts_meta=full_counts_meta,
        feature_report=predict_10k_align_report,
        matched_feature_genes=predict_10k_align_report.matched_feature_genes,
        missing_feature_genes=predict_10k_align_report.missing_feature_genes,
    )
    predict_5k_report = _compose_preprocess_report(
        config=preprocess_config,
        canonical_report=full_canonical_report,
        counts_meta=full_counts_meta,
        feature_report=predict_5k_align_report,
        matched_feature_genes=predict_5k_align_report.matched_feature_genes,
        missing_feature_genes=predict_5k_align_report.missing_feature_genes,
    )
    ...
    attach_preprocess_metadata(predict_10k_prepared, config=preprocess_config, report=predict_10k_report, feature_panel=feature_panel)
    attach_preprocess_metadata(predict_5k_prepared, config=preprocess_config, report=predict_5k_report, feature_panel=feature_panel)

    reference_10k_path = Path(str(manifest_10k["reference_h5ad"])).expanduser().resolve()
    query_10k_path = Path(str(manifest_10k["query_h5ad"])).expanduser().resolve()
    reference_5k_path = Path(str(manifest_5k["reference_h5ad"])).expanduser().resolve()
    query_5k_path = Path(str(manifest_5k["query_h5ad"])).expanduser().resolve()
    output_dir = reference_10k_path.parent
    output_dir.mkdir(parents=True, exist_ok=True)

    t0 = time.perf_counter()
    _write_adata(reference_prepared, reference_10k_path)
    _write_adata(reference_prepared, reference_5k_path)
    _write_adata(predict_10k_prepared, query_10k_path)
    _write_adata(predict_5k_prepared, query_5k_path)
    ...
    (output_dir / "preprocessing_summary.json").write_text(
        json.dumps(
            {
                "dataset_name": manifest_10k.get("dataset_name"),
                "source_h5ad": str(source_h5ad),
                "reference_output_h5ad_10k": str(reference_10k_path),
                "query_output_h5ad_10k": str(query_10k_path),
                "reference_output_h5ad_5k": str(reference_5k_path),
                "query_output_h5ad_5k": str(query_5k_path),
                "target_label": args.target_label,
                "split_key": args.split_key,
                "domain_key": args.domain_key,
                "build_size": int(args.build_size),
                "predict_size_10k": int(args.predict_size_10k),
                "predict_size_5k": int(args.predict_size_5k),
                "prep_manifest": str(Path(args.prep_manifest).expanduser().resolve()),
                "runtime_manifest_10k": str(Path(args.runtime_manifest_10k).expanduser().resolve()),
                "runtime_manifest_5k": str(Path(args.runtime_manifest_5k).expanduser().resolve()),
                ...
                "predict_10k_report": predict_10k_report.to_dict(),
                "predict_5k_report": predict_5k_report.to_dict(),
    ...
    (output_dir / "preparation_resource_summary.json").write_text(
        json.dumps(
            {
                "dataset_name": prep_manifest.get("dataset_name"),
                "source_h5ad": str(source_h5ad),
    ...
    print(
        json.dumps(
            {
                "reference_h5ad_10k": str(reference_10k_path),
                "query_h5ad_10k": str(query_10k_path),
                "reference_h5ad_5k": str(reference_5k_path),
                "query_h5ad_5k": str(query_5k_path),
                ...
                "preparation_resource_summary_json": str(output_dir / "preparation_resource_summary.json"),
            },
            indent=2,
            sort_keys=True,
        )
