...
PY
  [[ -n "${manifest}" ]] || continue
  out_dir="/tmp/atlasmtl_benchmarks/2026-03-09/multilevel_annotation/${dataset}/benchmark/gpu/${point}"
  mkdir -p "${out_dir}"
  if should_skip_point "${out_dir}"; then
    echo "=== skip existing multilevel gpu ${dataset} ${point} ==="
    continue
  fi
  echo "=== multilevel gpu ${dataset} ${point} ==="
