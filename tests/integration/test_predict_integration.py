...
def test_build_model_supports_adamw_and_plateau_scheduler_metadata():
    obs = pd.DataFrame({"anno_lv1": ["A", "A", "B", "B"]}, index=["r1", "r2", "r3", "r4"])
    ref = AnnData(X=np.array([[1, 0], [1, 1], [0, 1], [0, 2]], dtype=np.float32), obs=obs)
    ref.var_names = ["g1", "g2"]
    ...
    assert model.train_config["scheduler_patience"] == 5
...
def test_build_model_rejects_scheduler_without_validation_split():
    obs = pd.DataFrame({"anno_lv1": ["A", "A", "B", "B"]}, index=["r1", "r2", "r3", "r4"])
    ref = AnnData(X=np.array([[1, 0], [1, 1], [0, 1], [0, 2]], dtype=np.float32), obs=obs)
    ref.var_names = ["g1", "g2"]
